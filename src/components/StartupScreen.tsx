...
import airPressureReleaseSfx from "@/assets/sfx/air_pressure_release.mp3";
import sciFiDoorOpenSfx from "@/assets/sfx/sci_fi_door_open.mp3";
...
function StartupScreen() {
  ...
  const [playMusicSound, { sound: musicSoundObject }] = useSound(musicSound, {
    volume: audioVolume,
  });
  ...
  const leftOverlayPanelRef = useRef<HTMLDivElement>(null);
  const rightOverlayPanelRef = useRef<HTMLDivElement>(null);
  ...
  return (
    ...
    >
      ...
      <div ref={loadingTextContainerRef}>
        <h1
          className={cn(
            "text-2xl transition-opacity duration-500 ease-in-out select-none",
      ...
      <img
        ...
        className={cn(
          "w-screen h-screen object-cover absolute -z-10 brightness-0"
        )}
      ...
      {/* Start button */}
      <Button onClick={handleStartExperience}>{t`START`}</Button>
      ...
      {createPortal(
        <>
          <div
            ...
            className="h-screen w-[50vw] absolute left-0 -translate-x-full top-0 z-50 bg-white"
          />
          <div
            ref={rightOverlayPanelRef}
            className="h-screen w-[50vw] absolute right-0 translate-x-full top-0 z-50 bg-white"
          />
        </>,
        document.body
      )}
